...
#coding:utf-8
# Author:  guo
# Purpose: input: app path(signal app or app path)
#          output:analysed permissions 0101 saved in defined file("\\Output path for defaulty ")
# Changes : 
#     1. This main file(replace the apkscan.py)
#     2. lib\controller\extract_pms.py(replace the controller.py)
#     3. lib\core\common(add "save_pms_as_arff" function,save the 01 file to Output path)
#     4. lib\core\android(add "aapt_permissions", only extract permission info from a given app using "aapt" tool)
...
from lib.controller.extract_pms  import start
